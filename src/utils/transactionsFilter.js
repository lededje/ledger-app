...
export default function transactionsFilter(txs = [], actions, sortingOptions) {
  ...
  if(Object.keys(cleanedActions).length === 0) {
    ...
    filteredTx = _(txs).clone().filter((tx) => {
  ...
  const orderedTx =  _.orderBy(filteredTx, (tx) => sorts[sortingOptions.type](tx, sortingOptions.property), sortingOptions.ascending ? 'asc': 'desc');
...
export const sorts = {
  string: (tx, prop) => _.get(tx, prop),
  number: (tx, prop) => _.get(tx, prop),
  date: (tx, prop) => new Date(_.get(tx, prop)).getTime()
}
